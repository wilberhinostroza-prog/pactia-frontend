...
import { 
  ...
  confirmPayment,  // ← Agregar esta línea
  Contract,
  Payment,  // ← Agregar esta línea
...
// Función para cargar pagos pendientes de confirmación
const loadPendingConfirmations = async (phone: string) => {
  const myLent = await getActiveLent(phone);
    console.log('📋 Préstamos como acreedor:', myLent.length);
  const pending: { contract: Contract; payment: Payment }[] = [];
  
  myLent.forEach(contract => {
    console.log(`📋 Contrato ${contract.id} - Pagos:`, contract.payments?.length);
    contract.payments?.forEach(payment => {
      console.log(`   Pago ${payment.id}: confirmado = ${payment.confirmed}`);
      if (!payment.confirmed) {
        pending.push({ contract, payment });
      }
    });
  });
    console.log('⏳ Pagos pendientes encontrados:', pending.length);
...
const handleConfirmPayment = async (contractId: string, paymentId: string) => {
  try {
    ...
    await loadContracts(userPhone);
    await loadPendingConfirmations(userPhone);
  ...
  // Función para procesar pago con comprobante
  ...
  if (amount > realRemaining) {
    setToastMessage(`El monto no puede superar ${normalizers.currency(realRemaining)}`);
    setToastType('error');
    setToastVisible(true);
    return;
  }
  ...
  await executePay(
    makePayment(
      ...
      amount,
      attachedImage?.uri,    // ✅ Ya es URL de Supabase
  ...
  const remaining = originalAmount - confirmedTotal;
  return normalizers.currency(remaining);
};
...
// Verificar si hay pagos pendientes de confirmación
const hasPendingPayments = (contract: Contract): boolean => {
  return contract.payments?.some(p => !p.confirmed) || false;
};

  const getTypeIcon = (type: string) => type === 'prestamo' ? '💰' : '🤝';
  const getTypeTitle = (type: string) => type === 'prestamo' ? 'Préstamos' : 'Servicios';

  // Función para mostrar nombre + teléfono
  const getCounterpartyDisplay = (contract: Contract, isDebtor: boolean): string => {
    if (isDebtor) {
      return contract.creditorName 
        ? `${contract.creditorName} (${contract.creditorPhone})` 
        : contract.creditorPhone;
    } else {
      return contract.debtorName 
        ? `${contract.debtorName} (${contract.debtorPhone})` 
  ...
  const totalAmount = contracts.reduce((sum, c) => sum + (c.approvedAmount || c.requestedAmount || 0), 0);
  const totalRemaining = contracts.reduce((sum, c) => {
    const original = c.approvedAmount || c.requestedAmount || 0;
    ...
    >
      ...
      <View style={styles.cardFooter}>
        <Text style={styles.amount}>
          Monto: {normalizers.currency(contract.approvedAmount || contract.requestedAmount || 0)}
        </Text>
        <Text style={styles.dueDate}>
          Vence: {contract.approvedDueDate || contract.proposedDueDate}
        </Text>
      </View>

      <View style={styles.remainingContainer}>
        <Text style={styles.remainingText}>
          Saldo pendiente: {normalizers.currency(realRemaining)}
        </Text>
        {hasPending && (
      ...
      <View style={styles.sectionContainer}>
        <TouchableOpacity style={styles.sectionHeader} onPress={onToggle} activeOpacity={0.7}>
          ...
          <View style={styles.sectionStats}>
            ...
            <Text style={styles.sectionTotal}>Total: {normalizers.currency(stats.totalAmount)}</Text>
            <Text style={styles.sectionRemaining}>Pendiente: {normalizers.currency(stats.totalRemaining)}</Text>
  ...
  <View style={styles.uploadingOverlay}>
          ...
          <>
            ...
            <View style={styles.pendingPaymentHeader}>
              <Text style={styles.pendingPaymentDeudor}>
                Deudor: {contract.debtorName || contract.debtorPhone}
              </Text>
              <Text style={styles.pendingPaymentAmount}>
                {normalizers.currency(payment.amount)}
              </Text>
            </View>
            ...
            <View style={styles.pendingPaymentDetails}>
              <Text style={styles.pendingPaymentDate}>
                📅 Pagado el: {new Date(payment.date).toLocaleDateString('es-PE')}
              </Text>
    ...
    <View style={styles.emptyContainer}>
      ...
      {/* Modal para pago parcial con adjunto */}
      <Modal
        animationType="slide"
        transparent={true}
            ...
            {tempSelectedContract && (
              <>
                ...
                <Text style={styles.modalBalance}>
                  Saldo pendiente: {normalizers.currency(getRealRemainingAmount(tempSelectedContract))}
                </Text>
                
                {/* Campo de monto */}
                <View style={styles.modalInputGroup}>
                  <Text style={styles.modalLabel}>Monto a pagar (S/)</Text>
                  <TextInput
  ...
  >
                    <Text style={styles.modalConfirmText}>{attachedImage ? 'Pagar' : 'Adjunta un comprobante primero'}</Text>
                  </TouchableOpacity>
                </View>
              </>
            )}
